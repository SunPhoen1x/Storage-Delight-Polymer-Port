...
import com.phoen1x.block.entity.DrawerBooksBlockEntity;
...
public class DrawerBooksBlock extends BlockWithEntity implements FactoryBlock, InventoryProvider {
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return DrawerBooksBlockEntity::tick;
    }

    @Override
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (!player.isSneaking() && world.getBlockEntity(pos) instanceof DrawerBooksBlockEntity entity) {
    ...
    public static class Model extends BlockModel {
        ...
        private final ItemDisplayElement drawer_books;
        ...
        public Model(BlockState state, ServerWorld world, BlockPos pos, Identifier blockId) {
            ...
            this.drawer_books = state.get(OPEN) ? ItemDisplayElementUtil.createSimple(OPEN_TRUE) : ItemDisplayElementUtil.createSimple(OPEN_FALSE);
            this.drawer_books.setScale(new Vector3f(2.0f));
            this.updateStatePos(state);
            this.addElement(drawer_books);
        }

        private void updateStatePos(BlockState state) {
            var direction = state.get(FACING);
            var yaw = direction.getPositiveHorizontalDegrees();
            this.drawer_books.setYaw(yaw);
        ...
        private void updateBookshelfModel(BlockState state) {
            ...
            this.drawer_books.setItem(newModel);
            this.drawer_books.tick();
        }
