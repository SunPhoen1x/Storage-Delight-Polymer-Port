...
import com.phoen1x.block.DrawerBlock;
...
public class DrawerBlockEntity extends LockableBlockEntity implements MinimalSidedInventory, SidedInventory, BlockEntityExtraListener {
    ...
    public static <T extends BlockEntity> void tick(World world, BlockPos pos, BlockState state, T t) {
        if (t instanceof DrawerBlockEntity entity) {
    ...
    void setOpen(BlockState state, boolean open) {
        this.world.setBlockState(this.getPos(), state.with(DrawerBlock.OPEN, open), 3);
    }
    ...
    private class Gui extends LedgerSimpleGui {
        ...
        @Override
        public void onTick() {
            if (player.getPos().squaredDistanceTo(Vec3d.ofCenter(DrawerBlockEntity.this.pos)) > (18 * 18)) {
                this.close();
            }
            super.onTick();
        }
