...
import com.phoen1x.block.DrawerDoorBlock;
...
public class DrawerDoorBlockEntity extends LockableBlockEntity implements MinimalSidedInventory, SidedInventory, BlockEntityExtraListener {
    ...
    public static <T extends BlockEntity> void tick(World world, BlockPos pos, BlockState state, T t) {
        if (t instanceof DrawerDoorBlockEntity entity) {
    ...
    private final ViewerCountManager stateManager = new ViewerCountManager() {
        ...
        protected boolean isPlayerViewing(PlayerEntity player) {
            ...
            if (handler != null && !handler.slots.isEmpty()) {
                ...
                return slotInventory == DrawerDoorBlockEntity.this;
            }
    ...
    public void onListenerUpdate(WorldChunk chunk) {
        try {
            this.model = (DrawerDoorBlock.Model) BlockAwareAttachment.get(chunk, this.getPos()).holder();
        } catch (Throwable e) {
            StorageDelightPort.LOGGER.debug("Error updating drawer door model: ", e);
        }
    }
    ...
    void setOpen(BlockState state, boolean open) {
        this.world.setBlockState(this.getPos(), state.with(DrawerDoorBlock.OPEN, open), 3);
    }
    ...
    private class Gui extends LedgerSimpleGui {
        public Gui(ServerPlayerEntity player) {
            ...
            this.setTitle(Text.translatable("container.storagedelight.drawer_door"));
            for (int i = 0; i < 27; i++) {
                this.setSlotRedirect(i, new LedgerSlot(pos, player, DrawerDoorBlockEntity.this, i, 1, 1));
            }
            this.open();
        }
        ...
        @Override
        public void onTick() {
            if (player.getPos().squaredDistanceTo(Vec3d.ofCenter(DrawerDoorBlockEntity.this.pos)) > (18 * 18)) {
                this.close();
            }
            super.onTick();
        }
