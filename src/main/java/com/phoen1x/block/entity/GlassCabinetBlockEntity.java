...
import com.phoen1x.block.GlassCabinetBlock;
...
public class GlassCabinetBlockEntity extends LockableBlockEntity implements MinimalSidedInventory, SidedInventory, BlockEntityExtraListener {
    ...
    public static <T extends BlockEntity> void tick(World world, BlockPos pos, BlockState state, T t) {
        if (t instanceof GlassCabinetBlockEntity entity) {
    ...
    private final ViewerCountManager stateManager = new ViewerCountManager() {
        ...
        protected boolean isPlayerViewing(PlayerEntity player) {
            ...
            if (handler != null && !handler.slots.isEmpty()) {
                ...
                return slotInventory == GlassCabinetBlockEntity.this;
            }
    ...
    public void onListenerUpdate(WorldChunk chunk) {
        try {
            this.model = (GlassCabinetBlock.Model) BlockAwareAttachment.get(chunk, this.getPos()).holder();
        } catch (Throwable e) {
            StorageDelightPort.LOGGER.debug("Error updating glass cabinet model: ", e);
        }
    }
    ...
    void setOpen(BlockState state, boolean open) {
        this.world.setBlockState(this.getPos(), state.with(GlassCabinetBlock.OPEN, open), 3);
    }
    ...
    private class Gui extends LedgerSimpleGui {
        public Gui(ServerPlayerEntity player) {
            ...
            this.setTitle(Text.translatable("container.storagedelight.glass_cabinet"));
            for (int i = 0; i < 27; i++) {
                this.setSlotRedirect(i, new LedgerSlot(pos, player, GlassCabinetBlockEntity.this, i, 1, 1));
            }
            this.open();
        }
        ...
        @Override
        public void onTick() {
            if (player.getPos().squaredDistanceTo(Vec3d.ofCenter(GlassCabinetBlockEntity.this.pos)) > (18 * 18)) {
                this.close();
            }
            super.onTick();
        }
