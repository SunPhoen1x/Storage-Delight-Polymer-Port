...
import com.phoen1x.block.entity.CabinetVariantBlockEntity;
...
public class CabinetVariantBlock extends BlockWithEntity implements FactoryBlock, InventoryProvider {
    public static final MapCodec<CabinetVariantBlock> CODEC = createCodec(CabinetVariantBlock::new);
    public static final EnumProperty<Direction> FACING = Properties.HORIZONTAL_FACING;
    public static final BooleanProperty OPEN = Properties.OPEN;
    private Model model;

    public CabinetVariantBlock(Settings settings) {
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return CabinetVariantBlockEntity::tick;
    }

    @Override
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (!player.isSneaking() && world.getBlockEntity(pos) instanceof CabinetVariantBlockEntity entity) {
    ...
    public static class Model extends BlockModel {
        ...
        private final ItemDisplayElement cabinet;
        ...
        public Model(BlockState state, ServerWorld world, BlockPos pos, Identifier blockId) {
            ...
            this.cabinet = state.get(OPEN) ? ItemDisplayElementUtil.createSimple(OPEN_TRUE) : ItemDisplayElementUtil.createSimple(OPEN_FALSE);
            this.cabinet.setScale(new Vector3f(2.0f));
        ...
        private void updateCabinetModel(BlockState state) {
            ItemStack newModel = state.get(OPEN) ? OPEN_TRUE : OPEN_FALSE;
            this.cabinet.setItem(newModel);
            this.cabinet.tick();
        }
        ...
        public void notifyUpdate(HolderAttachment.UpdateType updateType) {
            if (updateType == BlockBoundAttachment.BLOCK_STATE_UPDATE) {
                updateCabinetModel(this.blockState());
